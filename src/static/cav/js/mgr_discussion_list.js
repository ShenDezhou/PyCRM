$(function() {
    ...
    $._changeSignUpStatus = function(id, status) {
        ...
        $.ajax({
            type: "POST",
            url: '/rest/mgr/discussion/update/signup/status',
            data: {
                _xsrf: $.cookie("_xsrf"),
                id: id,
                status: status
...
;
        if (src) {
            ...
            qrurl = '/rest/qrcode?link=' + '/page/discussion/' + id + '?src=' + channel_id + '&iconurl=' + channel_picture;

            $('#activity_qrcode_image').attr('src', qrurl);
            $('#modal-link-activity-link a[target=app_link]').attr('href', '/page/discussion/' + id + '?src=' + channel_id);
            $('#modal-link-activity-link textarea').val(location.protocol + '//' + location.hostname + '/page/discussion/' + id + '?src=' + channel_id);

        } else {
            qrurl = '/rest/qrcode?link=' + '/page/discussion/' + id;
            $('#activity_qrcode_image').attr('src', qrurl);
            $('#modal-link-activity-link a[target=app_link]').attr('href', '/page/discussion/' + id);
            $('#modal-link-activity-link textarea').val(location.protocol + '//' + location.hostname + '/page/discussion/' + id);
        }

    }

    $.show_link_modal = function(id, src, title) {
        $('#activity_preview a[target=app_link]').attr('href', '/page/discussion/' + id);
    ...
    $.stats_show_detail = function(activity_id, type, title) {
        if (detail_table == null) {
            detail_table = $('#detail_list_table').DataTable({
                ...
                "ajax": "/rest/mgr/discussion/sign_up/table?activity_id=" + activity_id + "&type=" + type,
                "columns": [{
                    "data": "fullname"
                }, {
                    "data": "created"
            ...
            detail_table.ajax.url("/rest/mgr/discussion/sign_up/table?activity_id=" + activity_id + "&type=" + type).load();
        }
    ...
    var show_dataTable = function() {
        if (current_table == null) {
            current_table = $('#data-table').DataTable({
                ...
                "ajax": "/rest/mgr/discussion/table?_xsrf=" + $.cookie('_xsrf'),
                "columns": [{
                    "data": "title"
                }, {
                    "data": "status",
                    ...
                    "data": "sign_up",
                    "mRender": function(data, something, full) {
                        var html = '<a class="btn btn-info btn-xs m-r-3" href="/console/discussion_sign_up_list?activity=' + full['id'] + '">' + data + '<i class="fa fa-fw fa-search"></i></a>';
                        // if (full.sur_id) {
                    ...
                    "data": "visits",
                    "mRender": function(data, something, full) {
                        var html = '<a class="btn btn-default btn-xs" href="/console/discussion_visit_list?activity=' + full['id'] + '">' + data + '<i class="fa fa-fw fa-search"></i></a>';
                        return html;
                    ...
                    "mRender": function(data, something, full) {
                        ...
                        var html = '<a class="btn btn-info btn-xs" target="ac_news" href="/page/discussion/' + full['id'] + '") style="margin-left:5px;">' + '预览' + '</a>';
                        ...
                        html += '<a class="btn btn-warning btn-xs" style="margin-left:5px;" href="/common/discussion_new?id=' + full['id'] + '">' + '修改' + '</a>';
            ...
            current_table.ajax.url("/rest/mgr/discussion/table?type=activity").load();
        }
    ...
    var get_channel_fields = function(id) {
        ...
        $.ajax({
            type: "GET",
            url: '/rest/mgr/discussion/detail',
            cache: false,
            data: {
                id: id
            },
        ...
        if (sub_table == null) {
            sub_table = $('#' + data_table_id).DataTable({
                ...
                "columns": [{
                    ...
                    "data": "picture",
                    "mRender": function(data, something, full) {
                        var qrurl = '/rest/qrcode?link=' + '/page/discussion/' + id + '?src=' + full["code_id"] + '&iconurl=' + full["picture"];
                    ...
                    "data": "code_id",
                    "mRender": function(data, something, full) {
                        var html = '<p class="text-info">' + location.protocol + '//' + location.hostname + '/page/discussion/' + id + '?src=' + data + '</p>';
                        return html;
                    }
                }, {
                    "data": "code_id",
    ...
    $("#save").click(function() {
        $.ajax({
            type: "POST",
            url: '/rest/mgr/discussion/edit/channel',
            data: {
                _xsrf: $.cookie("_xsrf"),
                id: event_id,
            ...
            success: function(data) {
                ...
                location.href = "/console/discussion_list";
            },
            error: function() {
                alert('网络错误！');
            },
    ...
    $('#btn_new').click(function() {
        window.location.href = "/common/discussion_new";
    });
});
