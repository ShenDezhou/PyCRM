$(function() {
    ...
    if (getQueryString('id') != null) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                for (var key in data) {
                    $('#channel_' + key).val(data[key]);
                }
                $('#channel_picture').attr('src', $.get_file_url(data['picture'], "/assets/ticket/image/"));
            },
            error: function() {
                alert('网络错误！');
            },
    ...
    $('#return').click(function() {
        location.href = "/console/channel_list";
    });
    var commitData = function() {
        ...
        // postData.type = type;
        var url = '';
        url = '/rest/mgr/channel/commit';
        if (id != "") {
            postData.id = id;
        }
        postData.name = $('#channel_name').val();
        if (id != "") {
            postData.picture = $('#channel_picture').attr('src').substr($('#channel_picture').attr('src').lastIndexOf("/") + 1)
        }
        if($('.preview-box img').attr('src')!=null)
            postData.picture = $('.preview-box img').attr('src').substr($('.preview-box img').attr('src').lastIndexOf("/") + 1)
    ...
    var initQuizModal = function(args) {
        if (!args) {
            ...
            for (var i = 0; i < args.opts.length; i++) {
                var id = args.opts[i].id,
                    sort = args.opts[i].sort,
                    title = args.opts[i].title;
    ...
    var updateQuizTable = function(id, title, type, sort, required, opts) {
        var optRowHtml = '<div class="opt-row" id="{id}"><span class="opt-sort hidden">{sort}</span>{index}.<span class="opt-title">{title}</span></div>',
            optHtml = '';
