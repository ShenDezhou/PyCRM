$(function() {
    ...
    $._changeStatus = function(id, status) {
        ...
        if (statu) {
            $.ajax({
                type: "POST",
                url: '/rest/mgr/article/update/status',
                data: {
                    _xsrf: $.cookie("_xsrf"),
                    id: id,
                    status: status
    ...
    var handleDataTableResponsive = function() {
        ...
        if ($("#news_list").length !== 0) {
                ...
                "ajax": "/rest/mgr/article/table?_xsrf=" + $.cookie('_xsrf'),
                "columns": [{
                    "data": "title"
                }, {
                    "data": "subject"
                ...
                "columnDefs": [{
                    "mRender": function(data, something, full) {
                        var html = '<a class="btn btn-info btn-xs" target="ac_news" onclick=$.show_link_modal("'+full['id']+'") style="margin-left:5px;">' + '详情' + '</a>';
                            html += '<a class="btn btn-warning btn-xs" style="margin-left:5px;" href="/console/article_new?id=' + full['id']+'">' + '修改' + '</a>';
    ...
    $('#btn_new').click(function() {
        window.location.href = "/console/article_new";
    });
});
