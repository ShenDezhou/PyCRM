...
$(function () {
    ...
    var org_id_set=getQueryString('org_id_set');
    console.log(org_id_set);

    if (person_id != null) {
        url = '/rest/mgr/person/member/detail';
    } else {
        url = '/rest/mgr/merge/org/info';
    }
    $.ajax({
        type: "get",
        cache: false,
...
$("#save").click(function () {
    ...
    if (member_type == 'person')
        ...
        url = '/page/merge/org/info/edit';
    $.ajax({
        type: "post",
        cache: false,
        url: url,
        data: $('#form_info').serialize()+'&org_id_set='+org_id_set, // 你的formid
        dataType: 'json',
        success: function (data) {
            if (data.error)
                alert(data.error);
