$(function() {
    ...
    $('#begin_discussion').click(function() {
        if (confirm("确定开始讨论吗？")) {
            var activity_id = getQueryString("activity");
            $.begin_discussion(activity_id);
        }
    });
    $.begin_discussion = function (activity_id) {
        var url  = '/rest/mgr/discussion/begin_discussion';
        $.ajax({
            type: "POST",
            url: url,
            data: {
            ...
            success: function(data) {
                ...
                if (confirm('是否发送报名成功确认短信？')) {
                    $.send_sms(activity_id, getAllCheckIds());
                } else {
                    alert(data.message);
    ...
    $('#end_discussion').click(function() {
        if (confirm("确定结束讨论吗？")) {
            var activity_id = getQueryString("activity");
            console.log(activity_id);
            $.end_discussion(activity_id);
        }
    });
    $.end_discussion = function (activity_id) {
        var url  = '/rest/mgr/discussion/end_discussion';
        $.ajax({
            type: "POST",
            url: url,
            data: {
    ...
    $.add_person_to_discussion = function (activity_id,id) {
        var url  = '/rest/mgr/discussion/add_person';
        $.ajax({
            type: "POST",
            url: url,
            data: {
    ...
    $.change_status = function(activity_id, person_id, status, reason) {
        var url  = '/rest/mgr/discussion/sign_up/status';
        $.ajax({
            type: "POST",
            url: url,
            data: {
    ...
    $.send_sms = function(activity_id, person_id) {
        $.ajax({
            type: "POST",
            url: '/rest/mgr/discussion/sign/up/sms',
            data: {
                _xsrf: $.cookie("_xsrf"),
                activity_id: activity_id,
                person_id: person_id
    ...
    $.stats_show_detail = function(activity_id, type, status_type) {
        ...
        if (detail_table == null) {
            detail_table = $('#detail_list_table').DataTable({
                ...
                "ajax": "/rest/mgr/discussion/sign_up/table?activity_id=" + activity_id + "&type=" + type + '&status_type=' + status_type,
                "columns": [{
                    "data": "fullname",
                    ...
                    "mRender": function(data, something, full) {
                        ...
                        html += '<a class="btn btn-success btn-xs" onclick=$.add_person_to_discussion("' + full['activity_id'] + '","' + full['person_id'] + '") style="margin-left:5px;">' + '添加到讨论组' + '</a>';
                        if (full.sur_id)
            ...
            detail_table.ajax.url("/rest/mgr/discussion/sign_up/table?activity_id=" + activity_id + "&type=" + type + '&status_type=' + status_type).load();
        }
    };
