$(function () {
    ...
    var handleDataTableResponsive = function () {
        ...
        if ($("#data-table").length !== 0) {
            table = $("#data-table").DataTable({
                ...
                "ajax": "/rest/mgr/merge/org/list?_xsrf=" + $.cookie('_xsrf'),
                "columns": [{
                    "data": "org_id"
                }, {
                    "data": "fullname"
                ...
                "columnDefs": [{
                    "mRender": function (data, type, full){
                        return '<input onclick="$._org_id_checkbox_click(\'' + full.org_id + '\')" type="checkbox" name="org_id" id="' + full.org_id + '" member_type="' + full.apply_member_type + '" />'
                    },
                    "targets": 0
                }, {
                    "mRender": function (data, type, full) {
                    ...
                    "mRender": function (data, type, full) {
                        ...
                        //html += '<a href="merge_org_info?org_id=' + full.org_id + '&org_id_set='+ org_id_set + '&redirect_url=' + location.pathname + '" class="btn btn-xs btn-warning m-r-5">合并至此</a>';
                        html += '<a onclick="$._jump(\'' + full.org_id + '\')"  class="btn btn-xs btn-warning m-r-5">合并至此</a>';
                        return html;
                    },
                    "targets": 6
                }],
    ...
    var data_reload = function (tag) {
        ...
        var url = "/rest/mgr/merge/org/list?_xsrf=" + parameter;
        console.log(url);
    ...
    $._showOrgDetail = function (id, detail_type) {
    ...
    $._org_id_checkbox_click = function(org_id){
        index=$.inArray(org_id,org_id_set);
        if ($('#'+org_id).attr('checked')) {
            if(-1==index){
                org_id_set.push(org_id);
            }
            console.log(org_id_set);
        } else{
            org_id_set.splice(index,1);
            console.log(org_id_set);
        }
    };
    $._jump = function(org_id){
        if($('#'+org_id).attr('checked')){
            org_id_set.splice($.inArray(org_id,org_id_set),1);
            for(i=0;i<org_id_set.length;i++){
                //console.log($('#'+org_id_set[i]).attr('member_type'));
                if("None"!=$('#'+org_id_set[i]).attr('member_type')){
                    alert('待合并企业中有企业会员，禁止合并！');
                    org_id_set.push(org_id);
                    return
                }
            }
            aim_url="merge_org_info?org_id=" + org_id + "&org_id_set="+ org_id_set + "&redirect_url=" + location.pathname;
            console.log(aim_url);
