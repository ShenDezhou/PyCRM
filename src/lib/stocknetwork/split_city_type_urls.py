...
STOP_WORD = [u'上海证券交易所',u'深圳证券交易所',u'北京时间']
if len(sys.argv) > 1:
	UrlCrumb = sys.argv[1]
...
def split_url_use_crumb(input, output, inCode='gb2312'):
	...
	for line in open(input):
		...
		if len(segs) < 3:
			continue
		url, crumb, major = segs[0].strip(), segs[1].strip() , segs[2].strip().replace(STOP_WORD[0],'').replace(STOP_WORD[1],'').replace(STOP_WORD[2],'')
		city = ""
