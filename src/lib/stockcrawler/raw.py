...
def save_line(line):
    ...
    with open(os.path.join(path,'save%s.txt' % ctime), 'ab') as fp:
        fp.write('%s\n' % (line.strip()))
        
def save_status(line):
    # ctime = time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()+8*3600))
    with open(os.path.join(path,'status.txt'), 'w+') as fp:
        fp.seek(0)
        fp.write('%s\n' % (line.strip()))
...
def read_status():
    ...
    with open(os.path.join(path,'status.txt'), 'r+') as fp:
        line= fp.readline()
        if len(line)==0:
            return 1;
        return int(line)    
...
def read_ip():
    # ctime = time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(time.time()+8*3600))
    with open(os.path.join(path,'ip.txt'), 'r+') as fp:
...
# if __name__ == '__main__':
    ...
    # count = 0
    # with open(os.path.join(path,'astock.txt'), 'r+') as fp:
    #     astockinfo= fp.readline().decode("UTF-8" )
    # with open(os.path.join(path,'sstock.txt'), 'r+') as fp:
    #     sstockinfo= fp.readline().decode("UTF-8" )
    # with open(os.path.join(path,'rstock.txt'), 'r+') as fp:
