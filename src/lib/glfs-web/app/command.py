...
def create_round_robin(nodes, max_dict_idx, volume_name, capacity, redundancy_ratio):
    ...
    for i in range(node_num):
        ...
        bricks += generate_bricks_per_node(node, volume_name, max_dict_idx[node])
    ...
    success, out = execute_gluster(VOLUME_CREATE + volume_name + disperse + bricks + FORCE)
...
def generate_bricks_per_node(hostname, volume_name, brick_num):
    ...
    cur_brick_record[hostname] = (BRICK_PER_NODE + cur_brick_index) % brick_num
