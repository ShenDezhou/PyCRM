...
class Connection( ):
    ...
    # @tornado.gen.coroutine
    # def executebatch(self, sqls):
    #     cur = self.cursor()
    #     res = False
    #     try:
    #         yield self.autocommit(False)
    #         for sql in sqls:
    #             yield cur.execute(sql)
    #         yield self.commit()
    #         yield self.autocommit(True)
    #         res = True
    #     finally:
    #         cur.close()
    #     raise Return(res)
    ...
    @tornado.gen.coroutine
    def executebatch(self, sqls, argsArray=None):
        cur = self.cursor()
        res = False
        try:
            yield self.autocommit(False)
            if argsArray:
                for sql, args in zip(sqls, argsArray):
    ...
    def callproc(self, proc, *args):
        ...
        cur = self.cursor()
        try:
            yield cur.callproc(proc, *args)
            if not cur.description:
                raise Return(False)
                return
            column_names = [d[0] for d in cur.description]
            res = [Row(zip(column_names, row)) for row in cur]
            if not res:
                raise Return(False)
                return
            raise Return(res)
    ...
    def next_result(self):
        self._affected_rows = yield self._read_query_result()
        raise Return(self._affected_rows)
        # return self._affected_rows
